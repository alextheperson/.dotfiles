lib: { performanceCores, efficiencyCores }:
let
  performanceCoresString = lib.strings.join "" (builtins.genList (x: ''{icon${builtins.toString x}}'') performanceCores);
  efficiencyCoresString = lib.strings.join "" (builtins.genList (x: ''{icon${builtins.toString (x + performanceCores)}}'') efficiencyCores);
in
''
  [
    {
      ...
      "position": "top",
      "height": 23,
      "modules-left": [ "custom/logo", "cpu", "memory", "temperature", "disk", "custom/powerDraw", "network#speed"],
      "modules-center": [ "clock" , "custom/weather"],
      "modules-right": [ "idle_inhibitor", "backlight", "pulseaudio", "custom/colorpicker", "bluetooth", "network#info", "battery" ],
...

    {
      ...
      "modules-right": [ "image#album-art",  "mpris" ],
      "reload_style_on_change":true,
      ...
      "cffi/niri-taskbar": {
        // I know it is weird, but if you use ~ here, it doesn't work
